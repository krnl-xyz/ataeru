...
contract RewardContract is RewardDataType {
    ...
    function addRewardMultiple(
        RewardData[] memory rewardDatas
    ) public returns (uint256[] memory _epochNumbers) {
    ...
    ) public returns (uint256 _epochNumber) {
        require(
            rewardData.startTime >= block.timestamp &&
                rewardData.endTime > rewardData.startTime,
