...
contract HRC is Callable, HRSDataStorage {
    ...
    event RegisteredWithHRC(
        address indexed _user,
        uint256 indexed _id,
        address indexed _hrs
    );
    ...
    function registerWithHRC(
        address user,
        uint256 _id,
        address _hrs
    ) public checkCanCall(msg.sender) {
    ...
    function requestBooking(
        ...
        require(
            requestContract.isRequestExist(_rid),
            "HRS__Request does not exist"
        );
        require(
            !requestContract.requestMaxDonor(_rid),
            "HRS__Request has max donors"
        );
    ...
    function acceptOrRejectBooking(
        address user,
        uint256 _rid,
        uint256 _id,
        SetParam memory setParam
    ) public checkCanCall(msg.sender) {
