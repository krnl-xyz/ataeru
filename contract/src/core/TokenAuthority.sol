...
contract TokenAuthority is Ownable {
    ...
    struct Krnl1591Response {
        bool message;
    }
    struct Krnl1592Response {
        string id;
        string facilityName;
        string city;
        string zip;
        ...
        string hospitalOverallRating;
        string hospitalOverallRatingFootnote;
    ...
    function _validateExecution(
        ...
        for (uint256 i = 0; i < _executions.length; i++) {
            if (_executions[i].kernelId == 1592) {
                Krnl1592Response memory result = abi.decode(
                    _executions[i].result,
                    (Krnl1592Response)
                );
                _executions[i].isValidated = true;
                _executions[i].opinion = true;
            }
            if (_executions[i].kernelId == 1591) {
                Krnl1591Response memory verified = abi.decode(
                    _executions[i].result,
                    (Krnl1591Response)
                );
                if (verified.message == false || verified.message == true) {
                    _executions[i].isValidated = true;
                    _executions[i].opinion = true;
                } else {
