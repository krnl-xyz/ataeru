...
contract AiAgent is DataStructures {
    ...
    function getAgentInfo(
        address agentAddress
    ) public view returns (AgentInfo memory) {
        return agents[msg.sender][agentAddress];
    }

    function getAgentPermission(
        address agentAddress
    ) public view returns (ActivityConfinment) {
    ...
    function _addAgentPermission(
        ...
        if (activity == ActivityConfinment.BOOKING) {
            canCall.setAppointee(
                user,
                agentAddress,
                address(hrs),
                HRC.requestBooking.selector
            );
        }
