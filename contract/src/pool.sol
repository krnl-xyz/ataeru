...
contract Pool is AccessControlUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    event PoolCreated(bytes32 indexed poolId, string name, address indexed poolAddress, PoolType indexed poolType);

    event PoolStatusChanged(bytes32 indexed poolId, PoolStatus oldStatus, PoolStatus newStatus);
    ...
    function createPool(string memory _name, address _poolAddress, PoolType _poolType, address[] memory _administrators)
        external
        ...
        returns (bytes32)
    {
        ...
        bytes32 poolId = keccak256(abi.encodePacked(_name, _poolAddress, block.timestamp));
    ...
    function updatePoolStatus(bytes32 _poolId, PoolStatus _newStatus) external onlyRole(POOL_MANAGER_ROLE) {
    ...
    function addPoolAdministrator(bytes32 _poolId, address _admin) external onlyRole(POOL_MANAGER_ROLE) {
        require(pools[_poolId].poolAddress != address(0), "Pool does not exist");
    ...
    function removePoolAdministrator(bytes32 _poolId, address _admin) external onlyRole(POOL_MANAGER_ROLE) {
        require(pools[_poolId].poolAddress != address(0), "Pool does not exist");
    ...
    function recordRequestProcessed(bytes32 _poolId, bytes32 _requestId, bool _approved) external {
        ...
        )
    {
        ...
        return (pool.name, pool.poolAddress, pool.poolType, pool.status, pool.totalRequests, pool.processedRequests);
    }
