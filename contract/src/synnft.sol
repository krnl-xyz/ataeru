...
contract SynNFT is ERC721Upgradeable, AccessControlUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    event HealthDataMinted(
        uint256 indexed tokenId, address indexed owner, string publicKey, bytes32 indexed hospitalId
    );

    event DataRequestSubmitted(
        bytes32 indexed requestId, uint256 indexed tokenId, address indexed requester, bytes32 poolId
    );

    event RequestVerified(bytes32 indexed requestId, address indexed verifier, uint256 verificationCount);

    event RequestApproved(bytes32 indexed requestId, uint256 indexed tokenId);

    event RequestRejected(bytes32 indexed requestId, string reason);

    event PoolCreated(bytes32 indexed poolId, address[] verifiers);

    event VerifierAddedToPool(bytes32 indexed poolId, address indexed verifier);
    ...
    function createVerificationPool(bytes32 _poolId, address[] memory _verifiers, uint256 _minimumVerifications)
        external
    ...
    function submitDataRequest(uint256 _tokenId, bytes32 _hospitalId, string memory _requestReason)
        external
        returns (bytes32)
    {
        ...
        bytes32 requestId = keccak256(abi.encodePacked(_tokenId, msg.sender, block.timestamp, _hospitalId));
    ...
    function getHealthDataInfo(uint256 _tokenId)
        external
        view
        returns (
        ...
        )
    {
        ...
        return (data.publicKey, data.dataHash, data.hospitalId, data.dataOwner, data.status, data.encryptedDataURI);
    }

    function getRequestStatus(bytes32 _requestId)
        external
        view
        returns (RequestStatus status, uint256 verificationCount, uint256 expiresAt, address requester)
    {
        DataRequest storage request = dataRequests[_requestId];
        return (request.status, request.verificationCount, request.expiresAt, request.requester);
    }
