...
contract VerificationPool is AccessControlUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    event VerificationRequested(
        bytes32 indexed requestId, address indexed requester, VerificationType indexed verificationType
    );
    ...
    event VerificationCompleted(bytes32 indexed requestId, VerificationStatus status);
    ...
    function initialize(address _mainPool, bytes32 _poolId, uint256 _threshold) public initializer {
    ...
    function submitVerificationRequest(
        ...
        bytes32 requestId = keccak256(abi.encodePacked(msg.sender, _dataHash, block.timestamp));
    ...
    function approveVerification(bytes32 _requestId, string memory _comment) external onlyRole(VERIFIER_ROLE) {
        VerificationRequest storage request = verificationRequests[_requestId];
        require(
            request.status == VerificationStatus.PENDING || request.status == VerificationStatus.IN_PROGRESS,
            "Invalid status"
        );
    ...
    function rejectVerification(bytes32 _requestId, string memory _reason) external onlyRole(VERIFIER_ROLE) {
        VerificationRequest storage request = verificationRequests[_requestId];
        require(
            request.status == VerificationStatus.PENDING || request.status == VerificationStatus.IN_PROGRESS,
            "Invalid status"
    ...
    function getVerifierComment(bytes32 _requestId, address _verifier) external view returns (string memory) {
