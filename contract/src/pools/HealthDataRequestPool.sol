...
contract HealthDataRequestPool is AccessControlUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function submitDataAccessRequest(
        ...
        bytes32 requestId = keccak256(
            abi.encodePacked(_tokenId, msg.sender, block.timestamp)
        );
    ...
    function createAccessGrant(bytes32 _requestId) internal returns (bytes32) {
        ...
        bytes32 grantId = keccak256(
            abi.encodePacked(_requestId, block.timestamp)
        );
    ...
    function revokeAccess(bytes32 _grantId, string memory _reason) external {
        ...
        require(
            msg.sender == request.dataOwner || hasRole(POOL_ADMIN_ROLE, msg.sender),
            "Unauthorized"
        );
    ...
    function rejectRequest(bytes32 _requestId, string memory _reason) 
        external 
