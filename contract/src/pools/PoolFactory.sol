...
contract PoolFactory is AccessControlUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function deployUserRegistrationPool() external onlyRole(FACTORY_ADMIN_ROLE) returns (address) {
        bytes32 poolId = keccak256(
            abi.encodePacked("USER_REGISTRATION", block.timestamp, msg.sender)
        );
    ...
    function deployHospitalRegistrationPool(
        address _hospitalContract
    ) external onlyRole(FACTORY_ADMIN_ROLE) returns (address) {
        bytes32 poolId = keccak256(
            abi.encodePacked("HOSPITAL_REGISTRATION", block.timestamp, msg.sender)
        );
    ...
    function deployHealthDataRequestPool(
        address _synNFTContract
    ) external onlyRole(FACTORY_ADMIN_ROLE) returns (address) {
        bytes32 poolId = keccak256(
            abi.encodePacked("HEALTH_DATA_REQUEST", block.timestamp, msg.sender)
        );
    ...
    function deployVerificationPool(
        uint256 _threshold
    ) external onlyRole(FACTORY_ADMIN_ROLE) returns (address) {
        require(_threshold > 0, "Invalid threshold");
        
        bytes32 poolId = keccak256(
            abi.encodePacked("VERIFICATION", block.timestamp, msg.sender)
        );
